...
grid = grid_function(1,0.001,0.01,1)
...
def compute_entropy(SDP):
    ...
    for time_step in range(len(gamma)):
        ...
        for a in range(len(F)):
            obj += -beta[time_step]*F[a]*C[a] - gamma[time_step]*C[a]
...
C = ncp.generate_operators('C', 3, hermitian=True)
...
for score in scores:
    ...
    ent = compute_entropy(sdp)
    entropy.append(1/np.log(2)*(2+ent))
    
    t2 = time.time()
    elapsed = t2 - t1
    times.append(elapsed)
    
    print(score, 1/np.log(2)*(2+ent), f"(time: {elapsed:.4f} s)")

# Create two different 2D arrays
results_entropy = np.column_stack((scores, entropy))
results_time = np.column_stack((scores, times))

# Save each separately
np.savetxt("entropy_results_ks_cglmp.txt", results_entropy, delimiter=",", header="score,entropy", comments="")
np.savetxt("time_results_ks_cglmp.txt", results_time, delimiter=",", header="score,time", comments="")
